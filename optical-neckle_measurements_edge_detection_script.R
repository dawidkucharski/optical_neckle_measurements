...
\pard\pardeftab720\partightenfactor0
\cf2 \
im_num = length(list.files("/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Optical_neckle_measurements/new_sensors/#9/1_000mm/"))\
c_num = 2\
\
while (c_num <= im_num) \{ \
\
file = file.path("/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Optical_neckle_measurements/new_sensors/#9/1_000mm/", \
                     paste("sample_1_000mm_1ch_", c_num, ".dat", sep=""))\
\
...
\
\
write.table(radius, file="/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Optical_neckle_measurements/new_sensors/#9/1_000mm/raw_signal_1ch.csv", append=TRUE,sep=",",col.names=FALSE,row.names=FALSE)\
  c_num = c_num+1\
\}\
neckle_data = read.csv("/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Optical_neckle_measurements/new_sensors/#9/1_000mm/raw_signal_1ch.csv", header = FALSE)\
...
\
\
tikz(file = "/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Publikacje w\uc0\u322 asne/Manuscripts/Optical_neckle_meas/figs/R_graphs/first_der9_990mm_1ch.tex", width = 4, height = 4)\
plot(pred1,col='black',lwd=2,lty=1, type="l", ylab= 
\f1\fs26 \cf5 expression\cf4 (d~y~\cf6 '/'\cf4 ~d~x)
\f0\fs24 \cf2 , xlab= ("Pixels"))\
grid(lty="dotted")\
...
\
\
tikz(file = "/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Publikacje w\uc0\u322 asne/Manuscripts/Optical_neckle_meas/figs/R_graphs/signal9_990mm_1ch.tex", width = 4, height = 4)\
plot(x, y, type="l", col=grey(.5), ylab=(expression(ADC[pixel]*" ["*a.u.*"]")), xlab= ("Pixels"))\
grid(lty="dotted")\
lines(x, y_sym_na.rm, col="black",lwd=2)\
lines(fit1,col='black',lwd=3,lty=3)\
...
\
%calibration\
one_channel<-data.frame(x = c(9.990,9,8,7,6), y=c(2334.05,2163.51,1946.58,1673.88,1481.15))\
plot(one_channel, xlab = "j", ylab = '',xlim=c(0,10), ylim=c(0,2500),pch=13, col="black", lwd=2,cex = 1.5)\
mod0 <- lm(formula = one_channel$y ~ 0+one_channel$x)\
grid(lty="dotted")\
plot(one_channel, xlab = expression(phi*" ["*mm*"]"), ylab = '',xlim=c(0,10), ylim=c(0,2500),pch=13, col="black", lwd=2,cex = 1.5)\
title(ylab = "px", mgp = c(2.5, 1, 1))\
grid(lty="dotted")\
abline(mod0,col="black", lwd=2,lty=1)\
\
\
\
\
...
\
\
%-----------------------------------------------------------Calibration graph_1ch-----------------------------------------------------------------------------------------\
tikz(file = "/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Publikacje w\uc0\u322 asne/Manuscripts/Optical_neckle_meas/figs/R_graphs/calibration_graph_1ch.tex", width = 4, height = 4)\
one_channel<-data.frame(x = c(9.990,9,8,7,6,5,4,3,2,1), y=c(2334.05,2163.51,1946.58,1673.88,1481.15,1235.27,1026.04,779.5,603.98,371.65))\
#plot(one_channel, xlab = "j", ylab = '',xlim=c(0,10), ylim=c(0,2500),pch=13, col="black", lwd=2,cex = 1.5)\
mod0 <- lm(formula = one_channel$y ~ one_channel$x)\
#grid(lty="dotted")\
plot(one_channel, xlab = expression(phi*" ["*mm*"]"), ylab = '',xlim=c(0,10), ylim=c(0,2500),pch=13, col="black", lwd=2,cex = 1.5)\
title(ylab = expression(px*" ["*a.u.*"]"), mgp = c(2.5, 1, 1))\
grid(lty="dotted")\
abline(mod0,col="black", lwd=2,lty=1)\
mtext("y=221.96x+141", side=4, line=-10, cex.lab=1,las=1, col="black")\
dev.off()\
as.formula(\
    paste0("y ~ ", round(coefficients(mod0)[1],2), " + ", \
           paste(sprintf("%.2f * %s", \
                         coefficients(mod0)[-1],  \
                         names(coefficients(mod0)[-1])), \
                 collapse=" + ")\
    )\
)\
a <- coefficients(mod0)[1] #intercept\
b <- coefficients(mod0)[2] #slope\
%-----------------------------------------------------------Calibration graph_1ch-----------------------------------------------------------------------------------------\
tikz(file = "/Users/DawidKucharski/Library/Mobile Documents/com~apple~CloudDocs/Dokumenty/Publikacje w\uc0\u322 asne/Manuscripts/Optical_neckle_meas/figs/R_graphs/calibration_graph_1ch.tex", width = 4, height = 4)\
one_channel<-data.frame(x = c(9.990,9,8,7,6,5,4,3,2,1), y=c(2364.59,2175.24,1879,1668.59,1477.27,1232.66,1030.93,875.46,505.7,276.01))\
#plot(one_channel, xlab = "j", ylab = '',xlim=c(0,10), ylim=c(0,2500),pch=13, col="black", lwd=2,cex = 1.5)\
mod0 <- lm(formula = one_channel$y ~ one_channel$x)\
#grid(lty="dotted")\
plot(one_channel, xlab = expression(phi*" ["*mm*"]"), ylab = '',xlim=c(0,10), ylim=c(0,2500),pch=13, col="black", lwd=2,cex = 1.5)\
...
grid(lty="dotted")\
abline(mod0,col="black", lwd=2,lty=1)\
mtext("y=228.36x+92.77", side=4, line=-10, cex.lab=1,las=1, col="black")\
dev.off()\
as.formula(\
    paste0("y ~ ", round(coefficients(mod0)[1],2), " + ", \
           paste(sprintf("%.2f * %s", \
